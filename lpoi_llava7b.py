...
from lpoi_dpo_trainer_5img import DPOTrainer
from lpoi_dpo_config import DPOConfig
from PIL import Image


# processor = AutoProcessor.from_pretrained("HuggingFaceM4/idefics2-8b", do_image_splitting=False)
processor = AutoProcessor.from_pretrained("llava-hf/llava-1.5-7b-hf", do_image_splitting=False, trust_remote_code=True, revision='a272c74')
...
def format(example):
    ...
    # full_path = full_path.replace("images", "dpo-5img")
    # negative_path = full_path.replace("images", "mdpo_negatives")
...
model = AutoModelForVision2Seq.from_pretrained("llava-hf/llava-1.5-7b-hf", torch_dtype=torch.float16, revision='a272c74', load_in_8bit=False)

peft_config = LoraConfig(target_modules="all-linear")
model = get_peft_model(model, peft_config)
model.print_trainable_parameters()
output_name = "llava-7b-lpoi-list5-10k"
